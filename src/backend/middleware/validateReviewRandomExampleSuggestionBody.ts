...
export const randomReviewExampleSuggestionSchema = Joi.array().items(Joi.object().keys({
  ...
  reviews: Joi.object().pattern(Joi.string(), Joi.string().valid(...Object.values(ReviewActions))),
}));
...
export default async (req: Interfaces.EditorRequest, res: Response, next: NextFunction): Promise<Response | void> => {
  ...
  try {
    await randomReviewExampleSuggestionSchema.validateAsync(finalData, { abortEarly: false });
    return next();
  } catch (err) {
    res.status(400);
    if (err.details) {
