...
export const wordDataSchema = Joi.object().keys({
  ...
  attributes: Joi.object().keys(
    Object.values(WordAttributes).reduce(
      (finalSchema, { value }) => ({
        ...finalSchema,
  ...
  stems: Joi.array().min(0).items(Joi.string()).allow(null).optional(),
  ...
  dialects: Joi.array()
    ...
    .items(
      Joi.object().keys({
        ...
        dialects: Joi.array()
          .min(0)
          .items(Joi.string().valid(...Object.keys(Dialects)))
          .required(),
