...
export const randomAudioExampleSuggestionSchema = Joi.array().items(Joi.object().keys({
...
export default async (req: Interfaces.EditorRequest, res: Response, next: NextFunction): Promise<Response | void> => {
  ...
  try {
    await randomAudioExampleSuggestionSchema.validateAsync(finalData, { abortEarly: false });
    return next();
  } catch (err) {
    res.status(400);
    if (err.details) {
