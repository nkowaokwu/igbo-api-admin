...
export const determineCreateSuggestionRedirection = async ({
  ...
  if (resource === Collection.WORDS) {
    prePopulateRecord.examples = compact(
      prePopulateRecord.examples.map((example) => {
        if (example.archived) {
          return null;
        }
  ...
  const finalResource =
    suggestionType === Collection.WORD_SUGGESTIONS || suggestionType === Collection.POLLS
      ? Collection.WORD_SUGGESTIONS
  ...
  const redirectLink = isPreExistingSuggestion ? `/${finalResource}/${suggestionId}/edit` : `/${finalResource}/create`;
  push({
