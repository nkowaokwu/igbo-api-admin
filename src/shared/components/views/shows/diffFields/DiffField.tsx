...
import { find, isEqual, split, map, isNaN } from 'lodash';
import { Box, Text, chakra, TextProps } from '@chakra-ui/react';
...
const DiffField = ({
  ...
  const renderValue = (value, hasChanged) => (renderNestedObject ? renderNestedObject(value, hasChanged) : value);
  ...
  return diffFromPath ? (
    diffFromPath.kind === DIFF_KEYS.Edit ? (
      <Box className="flex flex-col" data-test={`${path}-diff-field`}>
        <ValueLabel>
          {'Old: '}
          <chakra.span className="deletion-change">{renderValue(diffFromPath.lhs, true)}</chakra.span>
        </ValueLabel>
        <ValueLabel>
          {'New: '}
          <chakra.span className="addition-change">{renderValue(diffFromPath.rhs, true)}</chakra.span>
        </ValueLabel>
      </Box>
    ) : diffFromPath.kind === DIFF_KEYS.Array && diffFromPath.item.kind === DIFF_KEYS.New ? (
      <ValueLabel>
        {'New: '}
        <chakra.span className="addition-change">{renderValue(diffFromPath.item.rhs, true)}</chakra.span>
      </ValueLabel>
    ) : diffFromPath.kind === DIFF_KEYS.Array && diffFromPath.item.kind === DIFF_KEYS.Deletion ? (
      <ValueLabel>
        {'Deleting: '}
        <chakra.span className="deletion-change line-through">{renderValue(diffFromPath.item.lhs, true)}</chakra.span>
      </ValueLabel>
